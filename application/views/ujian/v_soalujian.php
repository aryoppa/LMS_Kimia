                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php foreach (['a', 'b', 'c', 'd', 'e'] as $index => $option): ?>
                                            <input type="radio" name="jawaban[<?= $s->id_soal_ujian; ?>]" value="<?= $option; ?>" required /> <?= chr(65 + $index) . ". " . $s->$option; ?><br>
                                            <?php $image_option = 'image_' . $option; ?>
                                            <img src="<?= base_url('uploads/'.$s->$image_option) ?>" alt="" width="300" onerror="this.onerror=null;" style="display: <?= empty($s->$image_option) ? 'none' : 'block'; ?>;"><br>
                                        <?php endforeach; ?>
                                        <br>
                                        <?= "Alasan Menjawab:" ?>
                                        <br>
