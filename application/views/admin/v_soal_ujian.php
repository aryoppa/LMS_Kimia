                    ...
                    <tbody>
                        <?php foreach ($soal_ujian as $index => $d): ?>
                            <tr>
                                    ...
                                    <ol type="A">
                                        <?php foreach (['a', 'b', 'c', 'd', 'e'] as $option): ?>
                                            <li<?= $d->kunci_jawaban == strtoupper($option) ? ' style="font-weight:bold;"' : ''; ?>>
                                                <?= $d->$option; ?>
                                                <?php $image_option = 'image_' . $option; ?>
                                                <img src="<?= base_url('uploads/'.$d->$image_option) ?>" alt="" width="100" onerror="this.onerror=null;" style="display: <?= empty($d->$image_option) ? 'none' : 'block'; ?>;">
                                            </li>
                                        <?php endforeach; ?>
                                    </ol>
                                </td>
                                <td>
                                    <strong><?= $d->kunci_jawaban; ?></strong>
                                    <?php $image_kunci = 'image_' . strtolower($d->kunci_jawaban); ?>
                                    <img src="<?= base_url('uploads/'.$d->$image_kunci) ?>" alt="" width="300" onerror="this.onerror=null;" style="display: <?= empty($d->$image_kunci) ? 'none' : 'block'; ?>;">
                                </td>
                                <td><ol type="A">
                                ...
                                <td>
                                    <?= $d->pembahasan; ?>
                                    <img src="<?= base_url('uploads/'.$d->image_pembahasan) ?>" alt="" width="300" onerror="this.onerror=null; this.src='<?= base_url('uploads/default.png') ?>';" style="display: <?= empty($d->image_pembahasan) ? 'none' : 'block'; ?>;">
                                    <?= $d->pembahasan_2; ?>
                                </td>
                                <td>
